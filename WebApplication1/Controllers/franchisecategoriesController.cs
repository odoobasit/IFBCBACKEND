...
[ApiController]
public class franchisecategoriesController : ControllerBase
{
    private readonly ApplicationDbContext _context;

    public franchisecategoriesController(ApplicationDbContext context)
    {
        _context = context;
    }
...
");
    ...
    // GET: api/FormData
    [HttpGet]
    public async Task<ActionResult<IEnumerable<franchisecategories>>> GetFormData()


    {
        Console.WriteLine(_context.franchisecategories);

        if (_context.franchisecategories == null)
        {
            return NotFound();
        }
        return await _context.franchisecategories.ToListAsync();
    }

    // GET: api/FormData/5
    [HttpGet("{id}")]
    public async Task<ActionResult<franchisecategories>> GetFormData(int id)
    {
        var formData = await _context.franchisecategories.FindAsync(id);
    ...
    // POST: api/FormData
    [HttpPost]
    public async Task<ActionResult<franchisecategories>> PostFormData(franchisecategories formData)
    {
        _context.franchisecategories.Add(formData);
        await _context.SaveChangesAsync();

        try
        {
            var candidateProfile = await _context.CandidateProfile
                                      .Where(u => u.docid == formData.docid) // Adjust the query as needed
            ...
            string FirstName = candidateProfile.firstName;
            string LastName = candidateProfile.lastName;
            string Email = candidateProfile.Email;
...
Email us at <a href=""mailto:info@ifbc.co"">info@ifbc.co</a><br/>
        ...
        <!--[if IE]></div><![endif]-->
                        ...
                        ";
            SendEmail(body, Email, FirstName + " " + LastName);

            string emailBody = "Candidate Name : "+ FirstName + " " + LastName +"<br><br>Email : " + Email;
...
>
  ...
  >
    ...
    // PUT: api/FormData/5
    [HttpPut("{id}")]
    public async Task<IActionResult> PutFormData(int id, franchisecategories formData)
    {
    ...
    public async Task<IActionResult> DeleteFormData(int id)
    {
        ...
        _context.franchisecategories.Remove(formData);
        await _context.SaveChangesAsync();
    ...
    {
        return _context.franchisecategories.Any(e => e.docid == id);
    }
}
