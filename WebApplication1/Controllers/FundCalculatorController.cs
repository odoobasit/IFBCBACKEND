...
[ApiController]
public class FundCalculatorController : ControllerBase
{
    private readonly ApplicationDbContext _context;

    public FundCalculatorController(ApplicationDbContext context)
    {
        _context = context;
    }
    ...
    // GET: api/FormData
    [HttpGet]
    public async Task<ActionResult<IEnumerable<FundCalculator>>> GetFormData()


    {
        if (_context.FundCalculator == null)
        {
            return NotFound();
        }
        return await _context.FundCalculator.ToListAsync();
    }

    // GET: api/FormData/5
    [HttpGet("{id}")]
    public async Task<ActionResult<FundCalculator>> GetFormData(int id)
    {
        var formData = await _context.FundCalculator.FindAsync(id);
    ...
    private void SendEmail(string body, string email, string name)
    {
        ...
        const string subject = "Ready to Fund Your Franchise? Check Out Your Customized Funding Plan!";
    ...
    public static string GenerateHtmlTable(FundCalculator formData)
    {
        var properties = typeof(FundCalculator).GetProperties();
        var htmlBuilder = new StringBuilder();
        string formattedDebtPayments = formData.DebtPayments.ToString("C", CultureInfo.CreateSpecificCulture("en-US"));
        string formattedHouseHold = formData.HouseHold.ToString("C", CultureInfo.CreateSpecificCulture("en-US"));
        string formattedDownPayment = formData.DownPayment.ToString("C", CultureInfo.CreateSpecificCulture("en-US"));
        string formattedTotalNet = formData.TotalNet.ToString("C", CultureInfo.CreateSpecificCulture("en-US"));
        ...
        foreach (var prop in properties)
        {
            ...
            if (propName != "DocId" && propName != "DocDate")
            {
                if (propName == "DebtPayments")
                {
                    propValue = formattedDebtPayments;
                }
                else if (propName == "HouseHold")
                {
                    propValue = formattedHouseHold;
                }
                else if (propName == "DownPayment")
                {
                    propValue = formattedDownPayment;
                }
                else if (propName == "TotalNet")
                {
                    propValue = formattedTotalNet;
                }
                htmlBuilder.Append($@"
                <table
    ...
    // POST: api/FormData
    [HttpPost]
    public async Task<ActionResult<FundCalculator>> PostFormData(FundCalculator formData)
    {

        _context.FundCalculator.Add(formData);
        await _context.SaveChangesAsync();
        string emailBody = GenerateHtmlTable(formData);
...
Email us at <a href=""mailto:info@ifbc.co"">info@ifbc.co</a><br/>
        ...
        <!--[if IE]></div><![endif]-->
                        ...
                        ";
        SendEmail(body, formData.Email, formData.FirstName);
        body = $@"
                <!DOCTYPE html PUBLIC ""-//W3C//DTD XHTML 1.0 Transitional //EN"" ""http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"">
    <html
      xmlns=""http://www.w3.org/1999/xhtml""
    ...
    // PUT: api/FormData/5
    [HttpPut("{id}")]
    public async Task<IActionResult> PutFormData(int id, FundCalculator formData)
    {
    ...
    public async Task<IActionResult> DeleteFormData(int id)
    {
        ...
        _context.FundCalculator.Remove(formData);
        await _context.SaveChangesAsync();
    ...
    {
        return _context.FundCalculator.Any(e => e.DocId == id);
    }
}
